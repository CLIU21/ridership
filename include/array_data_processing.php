...
function deep_matrix_differences($matrix1, $matrix2) {
	$answer = [];
	if ($matrix1 === $matrix2) {
		// echo "<h2>Arrays identical</h2>\n";
		return $answer;
	} elseif ($matrix1 == $matrix2) {
		echo "<h2>Arrays the same but not identical</h2>\n";
		return $answer;
	}
	if (count($matrix1) != count($matrix2)) {
		echo "<h2>Arrays of differing length</h2>";
	}
	$zip_matrix = array_map(null, $matrix1, $matrix2);
	foreach ($zip_matrix as list($row1, $row2)) {
		if ($row1 === $row2) {
			$answer[] = "same";
		} else {
			$zip_row = array_map(null, $row1, $row2);
			$row_diff = [];
			foreach ($zip_row as list($cell1, $cell2)) {
				if ($cell1 === $cell2) {
					$cell_diff = "same";
				} elseif ($cell1 == $cell2) {
					$cell_diff = "(same)";
				} else {
					$cell_diff = strval($cell1) . "(" . gettype($cell1) . ")"
					  . " != " . strval($cell2) . "(" . gettype($cell2) . ")";
				}
				$row_diff[] = $cell_diff;
			}
			$answer[] = $row_diff;
